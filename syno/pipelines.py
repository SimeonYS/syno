...
class SynoPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('syno.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `syno`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM syno WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO syno (date, title, link, content)"
